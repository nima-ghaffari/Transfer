...
# Helper class for a proper HTTPS Server
...
class ServerGUI(tk.Tk):
    ...
    def setup_styles(self):
        ...
        self.font_ip = ("Consolas", 12, "bold") # <<< فونت جدید برای IP

        style = ttk.Style(self)
        style.theme_use('clam')
        style.configure('.', background="#000F08", foreground="#F0F0F0", font=self.font_normal)
        style.configure('TLabel', background="#000F08", foreground="#F0F0F0")
        style.configure('TFrame', background="#000F08")
        style.configure('TLabelFrame', background="#0E1F18", borderwidth=1, relief='solid')
        style.configure('TLabelFrame.Label', background="#0E1F18", foreground="#1AAE95", font=self.font_bold, padding=(8, 4))
        style.configure('TNotebook', background="#000F08", borderwidth=0)
        style.configure('TNotebook.Tab', background="#136F63", foreground="#FFFFFF", font=self.font_bold, padding=[12, 6], borderwidth=0)
        style.map('TNotebook.Tab', background=[('selected', "#1AAE95"), ('active', "#136F63")])
        style.configure('TButton', background="#136F63", foreground="#FFFFFF", font=self.font_bold, padding=8, borderwidth=0, relief='flat')
        style.map('TButton', background=[('active', "#FFFFFF")], foreground=[('active', "#000000")])
        style.configure('Danger.TButton', background="#D32F2F", foreground="#FFFFFF")
        style.map('Danger.TButton', background=[('active', "#D32F2F")])
        style.configure('TEntry', fieldbackground="#03140D", foreground="#F0F0F0", insertcolor="#F0F0F0", borderwidth=2, relief='flat')
        style.map('TEntry', bordercolor=[('focus', "#1AAE95")], relief=[('focus', 'solid')])
        style.configure('Treeview', rowheight=25, fieldbackground="#03140D", background="#03140D", borderwidth=0, relief='flat')
        style.configure('Treeview.Heading', background="#136F63", foreground="#FFFFFF", font=self.font_bold, relief='flat', padding=6)
        style.map('Treeview.Heading', background=[('active', "#1AAE95")])
        style.map('Treeview', background=[('selected', "#1AAE95")], foreground=[('selected', "#FFFFFF")])
        style.configure('TRadiobutton', background="#0E1F18", font=self.font_normal)
        style.map('TRadiobutton', indicatorbackground=[('selected', "#1AAE95"), ('!selected', '#555')])
        style.configure('TCheckbutton', background="#0E1F18", font=self.font_normal)
    ...
    def create_control_tab(self, tab):
        ...
        path_label = ttk.Label(share_frame, textvariable=self.selected_path_var, font=self.font_normal, wraplength=500, foreground="#1AAE95", background="#0E1F18")
        ...
        ttk.Label(settings_frame, text="Port:", background="#0E1F18").grid(row=0, column=0, padx=5, pady=5, sticky='w')
        ...
        ttk.Label(settings_frame, text="Max Clients:", background="#0E1F18").grid(row=0, column=2, padx=(15, 5), pady=5, sticky='w')
        ...
        self.status_label = ttk.Label(control_status_frame, text="Status: Stopped", font=self.font_bold, foreground="#C74242")
        self.status_label.grid(row=0, column=2, sticky='w')
        
        # <<< استفاده از فونت جدید برای خوانایی بهتر IP >>>
        self.ip_label = ttk.Label(control_status_frame, text="Server IP: -", font=self.font_ip, foreground="#1AAE95")
        self.ip_label.grid(row=1, column=2, sticky='w', pady=(2,0))
        
        self.web_label = ttk.Label(control_status_frame, text="Web Access: -", font=self.font_normal)
        ...
        self.log_tree.column('Category', width=120, anchor='center')
        self.log_tree.column('Client IP', width=120, anchor='center')
    ...
    def create_management_tab(self, tab):
        ...
        self.clients_tree.column('Client IP', width=120, anchor='center')
        self.clients_tree.column('Status', width=120, anchor='center')
        self.clients_tree.column('Current File', width=180, anchor='w')
        self.clients_tree.column('Progress', width=80, anchor='center')
    ...
    def create_chat_tab(self, tab):
        ...
        self.chat_listbox = tk.Listbox(chat_list_frame, bg="#03140D", fg="#F0F0F0", relief='flat', borderwidth=0, font=self.font_bold, selectbackground="#1AAE95")
    ...
    def open_chat_window_for_client(self):
        ...
        chat_window.configure(bg="#000F08")
        chat_window.protocol("WM_DELETE_WINDOW", lambda ip=client_ip: self.on_chat_window_close(ip))
        chat_display = scrolledtext.ScrolledText(chat_window, state='disabled', wrap=tk.WORD, bg="#03140D", fg="#F0F0F0")
    ...
    def toggle_show_password(self):
        ...
        self.password_input.config(show=show_char)
    ...
    def confirm_password(self):
        ...
        self.confirm_password_btn.config(state='disabled')
        self.show_password_checkbox.config(state='disabled')
        self.toggle_btn.config(state='normal')
    ...
    def toggle_server(self):
        if not self.server.running:
            ...
            if self.require_password_var.get() and not self.confirmed_password:
                messagebox.showerror("Error", "Please confirm the password before starting the server.")
                return
            password_to_start = self.confirmed_password if self.require_password_var.get() else None
            try:
                port = int(self.port_input.get())
                max_clients = int(self.max_clients_input.get())
            except ValueError:
                messagebox.showerror("Error", "Port and Max Clients must be valid numbers.")
                return
            success, msg = self.server.start(port, self.selected_path_var.get(), self.share_mode_var.get(), password_to_start, max_clients)
            if success:
                self.toggle_btn.config(text="⏹️ Stop")
                self.pause_btn.config(state='normal')
                ip = self.server.get_local_ip()
                self.status_label.config(text="Status: Running", foreground="#1AAE95")
                self.ip_label.config(text=f"Server IP: {ip}")
                web_url = f"https://{ip}:{self.server.web_port}"
                self.web_label.config(text=f"Web Access: {web_url}")
                auth_mode = "Secure" if password_to_start else "Open"
                self.log_event(category="Server Status", details=f"Server ({auth_mode} Mode) started on {ip}:{port}")
            else:
                self.log_event(category="Error", details=msg)
                messagebox.showerror("Error", msg)
        else:
            self.server.stop()
            self.toggle_btn.config(text="▶️ Start")
            self.pause_btn.config(text='⏸️ Pause', state='disabled')
            self.status_label.config(text="Status: Stopped", foreground="#C74242")
            self.ip_label.config(text="Server IP: -")
            self.web_label.config(text="Web Access: -")
    ...
    def pause_server_activity(self):
        ...
        state_text = "Paused" if self.server.is_paused else "Resumed"
        self.pause_btn.config(text=f"{'▶️ Resume' if self.server.is_paused else '⏸️ Pause'}")
        self.log_event("Server Status", details=f"Server activity {state_text}.")

    def on_closing(self):
        if self.server.running:
            messagebox.showwarning("Server is Running", "Please stop the server before exiting.")
            return
        self.destroy()
    ...
    def disconnect_client(self):
        ...
        selected_item = self.clients_tree.selection()[0]
        client_ip = self.clients_tree.item(selected_item)['values'][0]
        if client_ip in self.server.clients_info:
            self.server.clients_info[client_ip]['socket'].close()
    ...
    def _log_event_gui(self, category, ip, details):
        ...
        self.log_tree.insert("", tk.END, values=tuple(log_entry.values()))
        self.log_tree.yview_moveto(1)
        
    def export_logs(self):
        if not self.log_data:
            messagebox.showinfo("No Logs", "There is no log data to export.")
            return
        ...
        try:
            if filepath.endswith('.xlsx'):
                pd.DataFrame(self.log_data).to_excel(filepath, index=False, engine='openpyxl')
            else:
    ...
    def remove_client_from_tree(self, ip):
        if ip in self.server.clients_info:
            tree_id = self.server.clients_info[ip].get('tree_id')
            if tree_id:
    ...
    def update_client_status(self, ip, status):
        if ip in self.server.clients_info and (tree_id := self.server.clients_info[ip].get('tree_id')):
            try: self.clients_tree.set(tree_id, 'Status', status)
            except tk.TclError: pass

    def update_client_file(self, ip, filename):
        if ip in self.server.clients_info and (tree_id := self.server.clients_info[ip].get('tree_id')):
            try: self.clients_tree.set(tree_id, 'Current File', filename)
            except tk.TclError: pass

    def update_client_progress(self, ip, progress):
        if ip in self.server.clients_info and (tree_id := self.server.clients_info[ip].get('tree_id')):
            try: self.clients_tree.set(tree_id, 'Progress', progress)
            except tk.TclError: pass

class FileServer:
    def __init__(self, gui):
        self.gui = gui
        self.host, self.port, self.chat_port, self.web_port = '0.0.0.0', 0, 0, 0
        ...
        try:
            ...
            if self.password:
                ...
                if password != self.password:
                    ...
                    self.gui.log_event("Authentication", client_ip, "Failed (Incorrect password).")
                    return
                client_socket.sendall(b"AUTH_SUCCESS")
                self.gui.log_event("Authentication", client_ip, "Successful.")
            else:
                client_socket.sendall(b'NO_PASS')
                self.gui.log_event("Authentication", client_ip, "Successful (No password).")
            ...
            while self.running:
                ...
                if command == 'LIST_FILES':
                    ...
                    for filename in requested_files:
                        ...
                        self.gui.update_client_status(client_ip, "Downloading")
                        self.gui.update_client_file(client_ip, filename)
                    ...
                    self.gui.update_client_status(client_ip, "Completed")
                    self.gui.update_client_file(client_ip, "-")
        except (socket.timeout, ConnectionResetError, ssl.SSLEOFError):
            self.gui.log_event("Error", client_ip, "Connection lost.")
        except Exception as e: 
            self.gui.log_event("Error", client_ip, f"An unexpected error occurred: {e}")
        finally:
